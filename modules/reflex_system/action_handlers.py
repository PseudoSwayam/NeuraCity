...
async def handle_security_call(location: str, source_module: Optional[str] = "unknown") -> dict:
    """Handles security dispatch, logs, records, and publishes a more SPECIFIC event type."""
    ...
    # We publish the new, more specific event type.
    await publisher.publish_event(event_type=event_type, payload=event_payload)
    
    return {"status": "success", "message": "Security team dispatched and event recorded."}

async def handle_announcement(message: str, source_module: Optional[str] = "unknown") -> dict:
    ...
    event_payload = {"message": message, "timestamp": datetime.datetime.now().isoformat(), "triggered_by": source_module}
...
async def handle_admin_notification(department: str, message: str, source_module: Optional[str] = "unknown") -> dict:
    ...
    event_payload = {"department": department, "message": message, "timestamp": datetime.datetime.now().isoformat(), "triggered_by": source_module}
